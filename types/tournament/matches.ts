  ...
  for (const round of tournament.rounds) {
    for (const match of round.matches) {
      if (match.team1 && !teamIds.has(match.team1.id)) {
        allTeams.push({ ...match.team1, wins: winsCounts[match.team1.id] || 0, losses: lossCounts[match.team1.id] || 0 });
        teamIds.add(match.team1.id);
      }
      if (match.team2 && !teamIds.has(match.team2.id)) {
        allTeams.push({ ...match.team2, wins: winsCounts[match.team2.id] || 0, losses: lossCounts[match.team2.id] || 0 });
