import type { Team } from "./tournament/mlb-teams";
...
export const advanceToNextRound = (tournament: Tournament): Tournament => {
  ...
  function createLosersBracketMatches() {
    ...
    // Round 4: First two losers of WB play at top of LB, bottom two at bottom of LB
    else if (nextRoundNumber === 4) {
      const losersFromWinnersBracket = justDroppedFromWinners.slice();
      
      if (losersFromWinnersBracket.length >= 4) {
        // Split winners bracket losers
        const topLosers = losersFromWinnersBracket.slice(0, 2);
        const bottomLosers = losersFromWinnersBracket.slice(2);
        ...
        // Place first two losers at top of bracket
        let matchCounter = 1;
        for (let i = 0; i < Math.min(topLosers.length, 2); i++) {
          const opponent = existingLosers[i] || null;
          nextRoundMatches.push(
            createMatch(
        ...
        // Add middle matches with remaining existing losers
        const middleExistingLosers = existingLosers.slice(2, existingLosers.length - 2);
        ...
        // Place bottom two losers at bottom of bracket
        for (let i = 0; i < Math.min(bottomLosers.length, 2); i++) {
          const opponentIndex = existingLosers.length - 2 + i;
          const opponent = existingLosers[opponentIndex] || null;
          nextRoundMatches.push(
            createMatch(
              `L${nextRoundNumber}-${matchCounter}`,
              nextRoundNumber,
    ...
    // Round 5: Two losers of WB play at bottom of LB
      ...
      if (losersFromWinnersBracket.length >= 2) {
        ...
        // Regular matches with existing losers at the top
        let matchCounter = 1;
        const topExistingLosers = existingLosers.slice(0, existingLosers.length - 2);
        for (let i = 0; i < topExistingLosers.length; i += 2) {
          const team1 = topExistingLosers[i];
          const team2 = topExistingLosers[i + 1] || null;
          nextRoundMatches.push(
            createMatch(
              `L${nextRoundNumber}-${matchCounter}`,
              nextRoundNumber,
        ...
        // Place both losers at bottom of bracket
        for (let i = 0; i < Math.min(losersFromWinnersBracket.length, 2); i++) {
          const opponentIndex = existingLosers.length - 2 + i;
          const opponent = existingLosers[opponentIndex] || null;
          nextRoundMatches.push(
            createMatch(
